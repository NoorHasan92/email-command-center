...
export function AdminClient({ runs, initialUsers }: { runs: RunWithResults[], initialUsers: UserWithCounts[] }) {
  ...
  const [waStatus, setWaStatus] = useState<"connecting" | "connected" | "logged_out" | "available">("available");
                  ...
                  <div className="shrink-0 flex items-center justify-center bg-black/40 p-4 rounded-xl border border-white/10 min-w-[250px] min-h-[250px]">
                    ...
                    {waStatus === "connecting" && waQr && (
                      <div className="bg-white p-3 rounded-xl shadow-inner text-center">
                        <QRCode
                          ...
                          fgColor="#000000"
                        />
                        <p className="text-xs text-muted-foreground mt-2">Scan with System WhatsApp</p>
                      </div>
                    )}
                    ...
                    {waStatus === "connected" && (
                      <div className="text-center w-full">
                        ...
                        <h3 className="font-semibold mb-1">System Connected</h3>
                        <p className="text-sm text-muted-foreground mb-4">{waMeta?.phoneNumber || "Ready to send"}</p>
                        <Button 
                          variant="destructive" 
                          className="w-full" 
